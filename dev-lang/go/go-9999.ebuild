...
src_compile()
{
	if use bootstrap
	then
		# Is there a better way to get the package version number?
		VERINS="$(best_version dev-lang/go-bootstrap)"
		VERINS="${VERINS/dev-lang\/go-bootstrap-/}"
		VERINS="${VERINS%.*}"
		export GOROOT_BOOTSTRAP="/usr/lib/go${VERINS}"
		[ ! -d "$GOROOT_BOOTSTRAP" ] && die "No such file or directory $GOROOT_BOOTSTRAP"
	else
